...
%> 
	...
	<div class="container-fluid px-4">
		<div id="title" class="d-flex justify-content-between">
			<h1 class="mt-4" onclick="location.href='/board/boardList.do?nowPage=1&cntPerPage=10&menu_id=${menu_id}'">게시판</h1>
	<%-- 		<c:if test="${empty loginId}">
		...
		<c:choose>
				...
				<li class="page-item">
					<a class="page-link" href="/board/boardList.do?nowPage=${p}&cntPerPage=${paging.cntPerPage}&search_type=${searchType}&search_word=${searchWord}&menu_id=${menu_id}"><b style="color:purple;">${p}</b></a>
				</li>
				</c:when>
				<c:when test="${p != paging.nowPage }">
				<li class="page-item">
					<a class="page-link" href="/board/boardList.do?nowPage=${p}&cntPerPage=${paging.cntPerPage}&search_type=${searchType}&search_word=${searchWord}&menu_id=${menu_id}">${p}</a>
				</li>
			</c:when>
		</c:choose>
			</c:forEach>
			    <c:if test="${paging.endPage lt paging.lastPage}">
					<li class="page-item">
						<a class="page-link" href="/board/boardList.do?nowPage=${paging.endPage+1}&cntPerPage=${paging.cntPerPage}&search_type=${searchType}&search_word=${searchWord}&menu_id=${menu_id}"aria-label="Next">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#searchBtn").on("click", function(){
			 let search_type = $("select[name='search_type'] option:selected").val();
				...
				let menuid = ${menu_id};
			location.href="/board/boardList.do?search_type="+search_type+"&search_word="+search_word+"&cntPerPage="+cntPerPage+"&menu_id="+menuid;
		})
		...
		$("#perPageBtn").on("click", function(){
			 ...
			 let menuid = ${menu_id};
			 location.href="/board/boardList.do?cntPerPage="+perPage+"&nowPage="+1+"&search_type="+searchType+"&search_word="+searchWord+"&menu_id="+menuid;
		})
	});
	...
	function openModal(e){
		...
		data.board_id = e;
		 $.ajax({
				type: "POST",
				url : "/board/boardNotice.do?board_id="+brdId+"&menu_id="+menuid,
	...
	function openBrd(e){
		...
		if(cateogy == 1){
			...
			var popupWidth = 600;
			var popupHeight = 500;
			let url = "/board/boardPost.do?board_id="+brdId+"&menu_id="+menuid;
			var popupX = Math.ceil(( window.screen.width - popupWidth )/2);
			var popupY = Math.ceil(( window.screen.height - popupHeight )/2); 
	...
	function openWrite(){
		...
		let url = "/board/boardRegister.do?&menu_id="+menuid;
		window.open( url, 'window_name', 'width=' + popupWidth + ',height=' + popupHeight + ',left='+ popupX + ', top='+ popupY);
