		...
		<div class="card mb-4">
			<div class="card-body">
				...
				<c:if test="${boardPost.writer eq loginId}">
					<button class="btn btn-outline-warning" onclick="location.href='/board/boardPostModify.do?board_id=${boardPost.board_id}&menu_id=${boardPost.menu_id}'">수정하기</button>
					<button class="btn btn-outline-danger" onclick="delPost()">삭제하기</button>
				</c:if>
				<c:if test="${not empty loginId}">
					<button class="btn btn-outline-info" onclick="location.href='/board/insertRePost.do?board_id=${boardPost.board_id}&menu_id=${menu_id}'">답글달기</button>
				</c:if>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	...
	function goList(){
		opener.call();
		location.href ="/board/boardList.do?menu_id="+menuid;
		window.close();
	}
	//게시글삭제
	function delPost(){
